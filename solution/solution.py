...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = f"Question: {question}\nAnswer: {answer}\nRubric: {rubric}\nPlease score the answer based on the rubric. Return JSON format."
        ...
        try:
            ...
            if match:
                ...
                parsed_scores = json.loads(response)
        except Exception:
            pass
            
        final_scores = {}
        for k in rubric:
            final_scores[k] = float(parsed_scores.get(k, 0.5))
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": sum(recall_values) / len(recall_values) if recall_values else None,
            "avg_context_precision": sum(precision_values) / len(precision_values) if precision_values else None,
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        failing_count = sum(1 for score in [f, r, c] if score < 0.5)
        if failing_count > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            sugg = suggestions[i] if i < len(suggestions) else "Investigate further"
            log += f"| {fid} | {ftype} | {cause} | {sugg} | Open |\n"
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Improve prompt clarity to better address the user's question")
        ...
        if categories.get("off_topic", 0) > 0:
            suggestions.append("Implement stricter intent detection to handle out-of-scope queries")
            
        default_suggestions = [
            "Add few-shot examples showing complete answers to improve completeness",
            "Tune retrieval thresholds to provide higher-quality context",
            "Evaluate LLM generation parameters like temperature for more factual output"
        ]
        
        for d in default_suggestions:
            if len(suggestions) >= max(3, len(failures)):
                break
